...
void `$INSTANCE_NAME`_set_channel(uint8_t channel)
{
    ...
    `$INSTANCE_NAME`_write_register(NRF_REG_RF_CH, channel);
...
{
    return `$INSTANCE_NAME`_read_register(NRF_REG_FIFO_STATUS);
}
...
{
    uint8_t count = `$INSTANCE_NAME`_read_register(NRF_REG_OBSERVE_TX);
...
{
    uint8_t lostPackets = `$INSTANCE_NAME`_read_register(NRF_REG_OBSERVE_TX);
...
{
    uint8_t pipe = `$INSTANCE_NAME`_read_register(NRF_REG_STATUS);
...
{
    `$INSTANCE_NAME`_write_register(NRF_REG_STATUS, NRF_ALL_IRQ_MASK);
}
...
{
    `$INSTANCE_NAME`_write_register(NRF_REG_STATUS, (1 << irq_flag));
}
