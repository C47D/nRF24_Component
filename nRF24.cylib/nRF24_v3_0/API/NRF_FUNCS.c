...
void `$INSTANCE_NAME`_start(void)
{
    ...
    `$INSTANCE_NAME`_cmd_flush_rx();
    `$INSTANCE_NAME`_cmd_flush_tx();
    // Clear IRQ flags
...
{
    mode == NRF_MODE_RX ? `$INSTANCE_NAME`_set_rx_mode() : `$INSTANCE_NAME`_set_tx_mode();
}
...
{
    `$INSTANCE_NAME`_set_bit(NRF_REG_CONFIG, NRF_CONFIG_BIT_PRIM_RX);
}
...
{
    `$INSTANCE_NAME`_clear_bit(NRF_REG_CONFIG, NRF_CONFIG_BIT_PRIM_RX);
}
...
{
    `$INSTANCE_NAME`_set_bit(NRF_REG_EN_AA, pipe);
}
...
{
    `$INSTANCE_NAME`_clear_bit(NRF_REG_EN_AA, pipe);
}
...
void `$INSTANCE_NAME`_set_channel(uint8_t channel)
{
    ...
    `$INSTANCE_NAME`_write_reg(NRF_REG_RF_CH, &channel, 1);
...
{
    uint8_t channel;
    `$INSTANCE_NAME`_read_reg(NRF_REG_RF_CH, &channel, 1);
    return channel;
}
...
uint8_t `$INSTANCE_NAME`_get_payload_size(const nrf_pld_size pipe)
{
    ...
    `$INSTANCE_NAME`_read_reg(pipe, &payload_size, 1);
    return payload_size;
}
...
{
    `$INSTANCE_NAME`_cmd_reuse_tx_payload();
...
void `$INSTANCE_NAME`_enable_dynamic_payload(void)
{
    ...
    `$INSTANCE_NAME`_set_bit(NRF_REG_FEATURE, NRF_FEATURE_BIT_EN_ACK_PAY);
    `$INSTANCE_NAME`_set_bit(NRF_REG_FEATURE, NRF_FEATURE_BIT_EN_DPL);
}
...
{
    `$INSTANCE_NAME`_set_bit(NRF_REG_EN_AA, pipe);
    `$INSTANCE_NAME`_set_bit(NRF_REG_DYNPD, pipe);
}
...
void `$INSTANCE_NAME`_disable_dynamic_payload()
{
    ...
    `$INSTANCE_NAME`_clear_bit(NRF_REG_FEATURE, NRF_FEATURE_BIT_EN_ACK_PAY);
    `$INSTANCE_NAME`_clear_bit(NRF_REG_FEATURE, NRF_FEATURE_BIT_EN_DPL);
}
...
{
    `$INSTANCE_NAME`_clear_bit(NRF_REG_EN_AA, pipe);
    `$INSTANCE_NAME`_clear_bit(NRF_REG_DYNPD, pipe);
}
...
{
    uint8_t fifo_status;
    `$INSTANCE_NAME`_read_reg(NRF_REG_FIFO_STATUS, &fifo_status, 1);
    return fifo_status;
}
...
{
    uint8_t count;
    `$INSTANCE_NAME`_read_reg(NRF_REG_OBSERVE_TX, &count, 1);
...
{
    uint8_t lostPackets;
    `$INSTANCE_NAME`_read_reg(NRF_REG_OBSERVE_TX, &lostPackets, 1);
...
{
    uint8_t pipe;
    `$INSTANCE_NAME`_read_reg(NRF_REG_STATUS, &pipe, 1);
...
{
    return `$INSTANCE_NAME`_read_bit(NRF_REG_RPD, NRF_RPD_BIT_RPD);
}
...
{
    return `$INSTANCE_NAME`_read_bit(NRF_REG_FIFO_STATUS, NRF_FIFO_STATUS_BIT_TX_FULL);
}
...
{
    return `$INSTANCE_NAME`_read_bit(NRF_REG_FIFO_STATUS, NRF_FIFO_STATUS_BIT_RX_EMPTY);
}
...
nrf_irq `$INSTANCE_NAME`_get_irq_flag(void)
{
    ...
    uint8_t sts = `$INSTANCE_NAME`_cmd_nop();
...
{
    uint8_t mask = NRF_ALL_IRQ_MASK;
    return `$INSTANCE_NAME`_write_reg(NRF_REG_STATUS, &mask, 1);
}
